...
const LovableUIPrompts = () => {
  ...
  useSEO({
    ...
    breadcrumbs: [{
      ...
      url: 'https://lovable.directory/lovable-prompts'
    }, {
      name: 'UI Prompts',
      url: 'https://lovable.directory/lovable-ui-prompts'
    }]
  });
  useEffect(() => {
    const fetchUIPrompts = async () => {
      ...
      const {
        data
      } = await supabase.from("prompts").select("*").or('title.ilike.%ui%,title.ilike.%design%,title.ilike.%sleek%,title.ilike.%hallucination%,description.ilike.%ui%,description.ilike.%interface%,tags.cs.{ui},tags.cs.{design}').ilike('title', '%lovable%').order("views_count", {
        ascending: false
      }).limit(30);
    ...
    const fetchFavorites = async () => {
      ...
      if (user) {
        const {
          data
        } = await supabase.from("favorites").select("prompt_id").eq("user_id", user.id);
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold">Lovable UI Prompts</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {uiCategories.map(cat => <Link key={cat.search} to={`/browse?search=${cat.search}+lovable`} className="flex items-center gap-2 p-4 rounded-lg border border-border hover:border-primary/50 hover:bg-accent/50 transition-colors">
          ...
          <section>
            ...
            {loading ? <div className="text-center py-12">
              ...
              </div> : prompts.length > 0 ? <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {prompts.map(prompt => <PromptCard key={prompt.id} id={prompt.id} slug={prompt.slug} title={prompt.title} description={prompt.description} category={prompt.category} tags={prompt.tags} viewsCount={prompt.views_count} favoritesCount={prompt.favorites_count} isFavorited={favoriteIds.has(prompt.id)} onFavoriteToggle={() => {}} />)}
